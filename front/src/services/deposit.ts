...
export default {
  create: async function (providerCode: string, receiverCode: string): Promise<CreateResponse> {
    ...
    return response.json()
      .then(({data}: ApiResponse<CreateResponse>) => data)
  },
  ...
  getById: async function (depositId: string): Promise<GetByIdResponse> {
    ...
    return response.json()
      .then(({data}: ApiResponse<GetByIdResponse>) => data)
  },

  getByIdAndLineId: async function (depositId: string, lineId: string): Promise<GetByIdAndLineIdResponse> {
    ...
    return response.json()
      .then(({data}: ApiResponse<GetByIdAndLineIdResponse>) => data)
  },

  addProduct: async function (depositId: string, productCode: string): Promise<AddProductResponse> {
    const response = await fetch(`http://localhost:8000/deposit/${depositId}/return/${productCode}`, {
      method: "GET",
      headers: {"content-type": "application/json;charset=UTF-8"}
    });

    return response.json()
      .then(({data}: ApiResponse<AddProductResponse>) => data)
  },
  ...
  cancelProduct: async function (depositId: string, lineId: string): Promise<void> {
    ...
    return response.json()
      .then(({data}: ApiResponse<void>) => data)
  },
